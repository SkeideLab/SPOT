...
PATH_v0 = "{prefix_model}_{param}_gradient_combat.gii"
    ...
    for area in ["V2v", "V2d", "V3v", "V3d"]:
        ...
        for hemi in ["L", "R"]:
            ...
            # FORMAT PATHS FOR INPUT AND OUTPUT
                ...
                combined_subject_data = np.vstack(parameters)

                #epsilon = 1e-6  # Small constant to avoid zero variance
                #combined_subject_data = combined_subject_data + epsilon
                save_raw_data = pd.DataFrame(combined_subject_data)
                save_raw_data.to_csv(f"/data/p_02915/SPOT/combat_hemi-{hemi}_area-{area}_{param}_{axis}.csv", index=False, header=False)
