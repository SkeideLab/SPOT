        ...
        # FORMAT PATHS FOR INPUT AND OUTPUT       
            ...
            for testdata in [R_v2v, A_v2v, S_v2v, R_v2d, A_v2d, S_v2d]:
                ...
                for _ in range(n_bootstraps):
                    ...
                    resampled_indices = np.random.choice(len(testdata), size=len(testdata), replace=True)
                ...
                test_statistic = np.abs(np.mean(bootstrap_statistics) - np.nanmean(testdata))
                bootstrap_mean_p_value = 2 * min(np.mean(bootstrap_statistics >= np.nanmean(testdata)), np.mean(bootstrap_statistics <= np.nanmean(testdata)))
                ...
                print(f"Bootstrap CI for mean value - Low: {bootstrap_ci_low}, High: {bootstrap_ci_high}")
