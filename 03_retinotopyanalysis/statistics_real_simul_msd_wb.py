...
def bootstrap_resample_wilcoxon(n_resamples, combined_data, n_groupA, n_groupB):
    ...
    for _ in range(n_resamples):
        ...
        bootstrap_mw_statistic, _ = wilcoxon(
...
for param in ["eccentricity", "polarangle"]:
    ...
    for group in ["12-16y", "18-21y", "neonates<37", "neonates>37", "fetal<29", "fetal>29"]:
            ...
            if group == "12-16y" or group == "18-21y":
                ...
                for model in ["real", "simulated"]:
                    for hemi in ["L", "R"]:
                        ...
                        # FORMAT PATHS FOR INPUT AND OUTPUT
                        prefix_model = PREFIX_MODEL.format(
                            sub=sub, ses=ses, hemi=hemi, model=model, param=param)
                        prefix_sub_template = PREFIX_SUB_TEMPLATE.format(
                            sub=sub, ses=ses, hemi=hemi)
                        # LOAD DATA
                        visparc = surface.load_surf_data(
                            PATH_VISPARC.format(
                                prefix_sub_template=prefix_sub_template)
        ...
        res = wilcoxon(groupA, groupB, alternative='two-sided',
                       method="approx")
        ...
        wilcoxon_results.append(
            ("real_benson", "real_simulated", mw_statistic, mw_p_value))
        print(
            f"Original wilcoxon test real_benson vs real_simulated - U statistic: {mw_statistic}, p-value: {mw_p_value}")
        ...
        with Pool(n_cores) as pool:
            ...
            results = pool.starmap(bootstrap_resample_wilcoxon, [(
        ...
        print(
            f"Bootstrapped Mann-Whitney U test benson vs simulated - p-value: {bootstrap_mw_p_value}")
