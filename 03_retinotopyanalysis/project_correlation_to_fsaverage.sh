...
for hemi in L R; do
    ...
    for model in simulated; do
        for param in r rss sigma v0i; do
            ...
            res_file_native="${path_output_dir}/ccfmodel/sub-${sub}/ses-${ses}/sub-${sub}_ses-${ses}_hemi-${hemi}_mesh-native_dens-native_desc-${model}_${param}.gii"
            res_file_fsaverage="${path_output_dir}/ccfmodel/sub-${sub}/ses-${ses}/sub-${sub}_ses-${ses}_hemi-${hemi}_mesh-fsaverage_dens-164k_desc-${model}_${param}.gii"
