...
for group in ["12-16y", "18-21y", "neonates<37", "neonates>37", "fetal<29", "fetal>29"]: 
    ...
    if group == "neonates<37":
        ...
        VISPARC_PATH = ("/data/p_02915/dhcp_derivatives_SPOT/fetal/dhcp_surface/sub-{sub}/ses-{ses}/anat/sub-{sub}_ses-{ses}_hemi-{hemi}_mesh-native_dens-native_desc-visualtopographywang2015_label-maxprob_dparc.label.gii")
        ...
        VISPARC_PATH = ("/data/p_02915/SPOT/00_HCP/template/hemi-{hemi}_mesh-native_dens-native_desc-visualtopographywang2015_label-maxprob_dparc.label.gii")
        ...
        if group == "12-16y" or group == "18-21y":
            for hemi in ["L", "R"]:
                ...
                if hemi == "L":
                    ...
                    standardized_squared_diff = squared_diff_data / voxel_variance[..., np.newaxis]

                    # Mean of standardized squared differences across voxels
                    mean_standardized_squared_diff = np.mean(standardized_squared_diff, axis=0)
                    ...
                    pd.DataFrame(sdvars, columns=['DVARS']).to_csv(output_path, index=False)
            ...
            for hemi in ["L", "R"]:        
                ...
                visparc = nib.load(VISPARC_PATH.format(hemi=hemi, sub=sub, ses=ses))
                indices_v2 = get_indices_roi(LABELS_V2, visparc)
                indices_v1 = get_indices_roi(LABELS_V1, visparc)
                prefix_model = PREFIX_MODEL.format(
                    sub=sub,
