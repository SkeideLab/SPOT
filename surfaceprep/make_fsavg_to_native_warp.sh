...
for hemi in left right; do
    ...
    wb_command -surface-sphere-project-unproject \
        ...
        $path_output_data/sub-$sub/ses-$ses/space-dhcpSym/surface_transforms/sub-${sub}_ses-${ses}_hemi-${hemi_upper}_from-native_to-fsaverage32k_dens-32k_mode-sphere_reg.surf.gii `# sphere-out: output sphere`

    # needed here as output: native sphere registered to fsaverage
    # 4. Resample retinotopy template onto native
    wang_template=$path_script/template_wang/hemi-${hemi_upper}_space-fsaverage_dens-164k_desc-visualtopographywang2015_label-maxprob_seg.label.gii
