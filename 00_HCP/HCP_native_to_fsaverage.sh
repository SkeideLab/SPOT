...
for hemi in L R; do
    ...
    for model in real simulated; do
        for param in eccentricity polarangle; do
            ...
            res_file_native="${path_output_dir}/ccfmodel_var/${sub}/${sub}_hemi-${hemi}_mesh-native_dens-native_label-${param}_desc-${model}_roi-v2th${threshold}_metric.gii"
            res_file_fsaverage="${path_output_dir}/ccfmodel_var/${sub}/${sub}_hemi-${hemi}_mesh-fsaverage_dens-164k_label-${param}_desc-${model}_roi-v2th${threshold}_metric.gii"
