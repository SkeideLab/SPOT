...
for hemi in L R; do
    ...
    for model in real simulated; do
        for param in eccentricity polarangle; do
            ...
            registration_from_native_to_fsaverage="/data/p_02915/dhcp_derivatives_SPOT/HCP-D/hcp_surface/${sub}/surface_transforms/${sub}_hemi-${hemi}_from-native_to-fsaverage164k_dens-32k_mode-sphere_reg.surf.gii"
            registration_from_fsaverage_to_native="/data/p_02915/dhcp_derivatives_SPOT/HCP-D/hcp_surface/${sub}/surface_transforms/${sub}_hemi-${hemi}_from-fsaverage_to-nativeT2w_dens-32k_mode-sphere_reg.surf.gii"
            native_midthickness="${path_anat_data}/${sub}.${hemi}.midthickness.native.surf.gii"

            res_file_native="${path_output_dir}/ccfmodel_7T_32k/${sub}/${sub}_hemi-${hemi}_mesh-native_dens-native_label-${param}_desc-${model}_roi-v2th${threshold}_metric.gii"
            res_file_fsaverage="${path_output_dir}/ccfmodel_7T_32k/${sub}/${sub}_hemi-${hemi}_mesh-fsaverage_dens-164k_label-${param}_desc-${model}_roi-v2th${threshold}_metric.gii"
