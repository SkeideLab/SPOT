...
for hemi in left right; do
    ...
    registration_from_native_to_fsaverage="$path_output_data/$sub/surface_transforms/${sub}_hemi-${hemi_upper}_from-native_to-fsaverage164k_dens-164k_mode-sphere_reg.surf.gii"
    ...
    # output
    path_out_native_hemi="$path_out_native/${sub}_hemi-${hemi_upper}_mesh-native_dens-native"
    ...
    wb_command -surface-sphere-project-unproject \
        "/data/pt_02880/HCP_D/fmriresults01/${sub}/MNINonLinear/Native/${sub}.${hemi_upper}.sphere.reg.reg_LR.native.surf.gii" `# sphere-in: sphere with desired output mesh: dhcpsym registered to fslr` \
        $path_HCPtemplates_standardmeshatlases/${hemi_upper}.sphere.164k_fs_LR.surf.gii `# sphere-project-to: sphere that aligns with sphere-in: fs_lr` \
        $path_HCPtemplates_standardmeshatlases/resample_fsaverage/fs_LR-deformed_to-fsaverage.${hemi_upper}.sphere.164k_fs_LR.surf.gii `# sphere-unproject-from: sphere-project-to deformed to desired output space: fs_lr registered to fsaverage` \
