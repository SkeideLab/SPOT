...
groups = ["fetal<29", "fetal>29", "neonates<37", "neonates>37", '12-16y', '18-21y']
...
left_label = {'neonates<37': 'c', 'neonates>37': 'd', 'fetal<29': 'a', 'fetal>29': 'b', '12-16y': 'e', '18-21y': 'f'}
for i, group in enumerate(groups):
    ...
    hist_R_pos = [0.60, 1 - i*0.15, 0.12, 0.1]  # Adjusted position for the second histogram
        
    ax_hist_L = fig.add_axes(hist_L_pos)
    ax_hist_R = fig.add_axes(hist_R_pos)
    
    sns.histplot(data=data_L, x='ecc', ax=ax_hist_L, color=colors[group], bins=bin, stat='density')
    ax_hist_L.set_xlim(0, 23)
    ax_hist_L.set_ylim(0, 0.25)
    ax_hist_L.set_xlabel("Eccentricity" if i == 5 else "", labelpad=1)
    ax_hist_L.set_ylabel("Probability density" if i == 5 else "",)
    
    sns.histplot(data=data_R, x='ecc', ax=ax_hist_R, color=colors[group], bins=bin, stat='density')
    ax_hist_R.set_xlim(0, 23)
    ax_hist_R.set_ylim(0, 0.25)
    ax_hist_R.set_xlabel("Eccentricity" if i == 5 else "", labelpad=1)
    ax_hist_R.set_ylabel("Probability density" if i == 5 else "",)

    fig.text(0.08, 1 - (i * 0.15) + 0.11, left_label[group], ha='center', va='center', fontsize=12, weight='bold')
    ...
    fig.text(0.395, 1 - (i * 0.15) + 0.12, text_label[group], ha='center', va='center', fontsize=14)
...
# Add colorbar
cbar_ax = fig.add_axes([0.24, 0.18, 0.3, 0.02])  # Adjust the position and height of the colorbar axis
cbar = plt.colorbar(cm.ScalarMappable(cmap='jet', norm=plt.Normalize(vmin=0, vmax=20)), cax=cbar_ax, orientation='horizontal', fraction=.1)
cbar.set_label('Eccentricity')

# Save the figure
fig.savefig("/data/p_02915/dhcp_derivatives_SPOT/Figures_v2/ECC_all.png", dpi=300, bbox_inches='tight')
