...
for axis in axes: 
    ...
    for hemi_index, hemi in enumerate(hemispheres):
        ...
        for area_index, area in enumerate(areas):  
            combat_harmonized = pd.read_csv(f"/data/p_02915/SPOT/raw_hemi-{hemi}_area-{area}_{param}_{axis}.csv", index_col=None).to_numpy()
            if area =="V2v":
                title = "ventral"
                raw_harmonized = pd.read_csv(f"/data/p_02915/SPOT/raw_hemi-{hemi}_area-V3v_{param}_{axis}.csv", index_col=None).to_numpy()
            elif area =="V2d":
                raw_harmonized = pd.read_csv(f"/data/p_02915/SPOT/raw_hemi-{hemi}_area-V3d_{param}_{axis}.csv", index_col=None).to_numpy()
                title = "dorsal"
    ...
    plt.tight_layout()
    #plt.show()
    fig.savefig(f"/data/p_02915/dhcp_derivatives_SPOT/Figures_v3/gradient_raincloud_{param}.png", dpi = 300)