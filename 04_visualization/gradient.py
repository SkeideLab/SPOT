    ...
    for group in ["neonates<37", "neonates>37", "fetal<29", "fetal>29", "12-16y", "18-21y"]:
        ...
        for hemi in ["L", "R"]:
            ...
            # FORMAT PATHS FOR INPUT AND OUTPUT       
                ...
                normalized_gradients = gradients / gradient_magnitude[:, np.newaxis]
                # Create vectors for arrows using gradient directions and magnitudes
                vectors = normalized_gradients * gradient_magnitude[:, np.newaxis]
                ...
                arrow_start = np.array(mesh_delaunay.center)  # Example arrow start point                
